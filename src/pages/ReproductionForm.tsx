...
export function ReproductionForm() {
  ...
  return <div className="mx-auto max-w-xl space-y-5"><div className="flex items-center gap-3"><button onClick={() => navigate(-1)} className="rounded-xl p-2 text-[#516058] hover:bg-white"><ArrowLeft size={18} /></button><div><p className="page-kicker">Reprodução</p><h1 className="mt-1 text-2xl font-bold tracking-tight text-[#17231b]">Registrar gestação</h1></div></div><form onSubmit={handleSubmit} className="app-surface space-y-5 p-5 sm:p-7"><div><label className={labelClass}>Fêmea *</label><select value={motherId} onChange={event => setMotherId(event.target.value)} required className={inputClass}><option value="">Selecione a fêmea</option>{mothers.map(mother => <option key={mother.id} value={mother.id}>{mother.name ?? mother.tag ?? 'Sem nome'}</option>)}</select></div><div><label className={labelClass}>Quando pegou cria? *</label><input type="text" inputMode="numeric" maxLength={10} value={breedingDate} onChange={event => setBreedingDate(formatMask(event.target.value))} className={inputClass} placeholder="dd/mm/aaaa" /></div>{expectedBirth && <div className="rounded-2xl bg-brand-50 p-4"><div className="flex gap-3"><span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-white text-brand-700"><CalendarHeart size={18} /></span><div><p className="text-sm font-bold text-[#254331]">Previsão de parto</p><p className="mt-0.5 text-sm text-brand-700">{formatDate(expectedBirth)}</p><p className="mt-1 text-xs text-[#678070]">Cálculo estimado em 283 dias de gestação.</p></div></div></div>}<div><label className={labelClass}>Observações</label><textarea value={notes} onChange={event => setNotes(event.target.value)} className={inputClass} rows={3} placeholder="Ex.: inseminação artificial, touro utilizado..." /></div>{error && <p className="rounded-xl border border-red-100 bg-red-50 px-3 py-2.5 text-sm text-red-700">{error}</p>}<div className="flex flex-col-reverse gap-3 border-t border-[#e7eee9] pt-5 sm:flex-row sm:justify-end"><button type="button" onClick={() => navigate(-1)} className="rounded-xl px-5 py-2.5 text-sm font-semibold text-[#526158] hover:bg-[#f1f5f2]">Cancelar</button><button type="submit" disabled={loading} className="rounded-xl bg-brand-700 px-5 py-2.5 text-sm font-semibold text-white shadow-[0_8px_18px_rgba(31,73,51,.18)] hover:bg-brand-800 disabled:opacity-60">{loading ? 'Salvando...' : 'Registrar gestação'}</button></div></form></div>
}
