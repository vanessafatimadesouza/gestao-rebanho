...
import { AlertCircle, ArrowRight, Baby, CalendarDays, CalendarHeart, Check, ChevronRight, Leaf, Lightbulb, ListChecks, PawPrint, Search, ShieldCheck, Sprout, Syringe, Tag } from 'lucide-react'
...
export function Dashboard() {
  ...
  return <div className="space-y-6">
    ...
    <section><div className="mb-4"><SectionTitle>Ações rápidas</SectionTitle></div><div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><QuickAction to="/animais" label="Consultar rebanho" detail="Acessar todos os animais" icon={Search} /><QuickAction to="/animais/novo" label="Cadastrar animal" detail="Adicionar um novo animal ao rebanho" icon={Tag} /><QuickAction to="/vacinas/nova" label="Registrar vacina" detail="Aplicar vacina ou reforço" icon={ShieldCheck} /><QuickAction to="/partos/novo" label="Registrar parto" detail="Incluir uma nova cria" icon={CalendarHeart} /></div></section>
    ...
    <section className="grid items-stretch gap-5 lg:grid-cols-2">
      <div className="flex h-full flex-col rounded-[28px] border border-[#dfeae2] bg-white p-6 shadow-[0_12px_30px_rgba(22,60,40,.045)]">
        <div className="flex flex-wrap items-start justify-between gap-3"><div className="flex items-start gap-3"><span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-[#eaf6ee] text-[#267149]"><ListChecks size={21} aria-hidden="true" /></span><div><h2 className="text-lg font-bold tracking-tight text-[#162c20]">Prioridades de hoje</h2><p className="mt-0.5 text-xs text-[#728178]">O que merece sua atenção no rebanho</p></div></div><Link to="/vacinas" className="inline-flex min-h-9 items-center gap-1.5 rounded-full bg-[#f0f7f2] px-3 text-xs font-bold text-[#276543] transition-colors hover:bg-[#e2f1e6]">Ver todos <ArrowRight size={14} aria-hidden="true" /></Link></div>
        <div className={`mt-5 flex items-center gap-4 rounded-2xl border p-4 sm:p-5 ${upcoming.length ? 'border-[#f2dfbb] bg-[#fff9ed]' : 'border-[#cfe8d7] bg-[linear-gradient(110deg,#eaf8ef,#f7fcf8)]'}`}><span className={`flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl ${upcoming.length ? 'bg-[#fff0d5] text-[#a86b16]' : 'bg-white text-[#21845a]'}`}>{upcoming.length ? <AlertCircle size={28} strokeWidth={1.8} aria-hidden="true" /> : <ShieldCheck size={28} strokeWidth={1.8} aria-hidden="true" />}</span><div className="min-w-0"><p className={`text-xs font-bold uppercase tracking-[0.12em] ${upcoming.length ? 'text-[#a86b16]' : 'text-[#318155]'}`}>{upcoming.length ? 'Atenção' : 'Tudo em dia'}</p><h3 className="mt-0.5 text-base font-bold text-[#173326]">{upcoming.length ? `${upcoming.length} ${upcoming.length === 1 ? 'vacina próxima' : 'vacinas próximas'}` : 'Nenhuma vacina pendente'}</h3><p className="mt-0.5 text-xs leading-5 text-[#5c7062]">{upcoming.length ? 'Confira os vencimentos dos próximos 30 dias.' : 'Não há doses com vencimento nos próximos 30 dias.'}</p></div></div>
        <div className="mt-3 space-y-2">{upcoming.slice(0, 3).map(vaccination => <Priority key={vaccination.id} icon={Syringe} title={vaccination.vaccine_name} detail={`Vencimento: ${vaccination.next_due_date ? new Date(vaccination.next_due_date + 'T12:00:00').toLocaleDateString('pt-BR') : 'a confirmar'}`} alert />)}{(stats?.recentBirths ?? 0) > 0 ? <Priority icon={Baby} title="Partos registrados" detail={`${stats?.recentBirths} registro(s) nos últimos 30 dias`} /> : <Priority icon={ListChecks} title="Verificações de manejo" detail="Nenhum lembrete pendente" />}</div>
      </div>
      <div className="flex h-full flex-col rounded-[28px] border border-[#dfeae2] bg-white p-6 shadow-[0_12px_30px_rgba(22,60,40,.045)]">
        <div className="flex items-start gap-3"><span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-[#f0f5e9] text-[#4c7852]"><Lightbulb size={21} aria-hidden="true" /></span><div><h2 className="text-lg font-bold tracking-tight text-[#162c20]">Insights do manejo</h2><p className="mt-0.5 text-xs text-[#728178]">Pequenos cuidados, melhores decisões</p></div></div>
        <div className="mt-5 grid flex-1 gap-2"><Insight icon={Leaf} number="01" title="Rebanho acompanhado" detail={`${stats?.activeAnimals ?? 0} animais ativos. Mantenha os cadastros atualizados.`} /><Insight icon={Syringe} number="02" title={upcoming.length ? 'Vacinação pede atenção' : 'Vacinação em dia'} detail={upcoming.length ? `${upcoming.length} ${upcoming.length === 1 ? 'vacina precisa' : 'vacinas precisam'} de atenção nos próximos 30 dias.` : 'Nenhuma vacina prevista para os próximos 30 dias.'} /><Insight icon={Sprout} number="03" title="Registre os manejos" detail="Um histórico completo melhora a gestão da fazenda." /></div>
      </div>
    </section>
  </div>
}
...
function Priority({ icon: Icon, title, detail, alert = false }: { icon: typeof Syringe; title: string; detail: string; alert?: boolean }) { return <div className="flex min-h-[66px] items-center gap-3 rounded-2xl border border-[#e8efe9] bg-white p-3"><span className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-xl ${alert ? 'bg-[#fff3db] text-[#b57919]' : 'bg-[#f0f7f2] text-[#28734a]'}`}><Icon size={18} aria-hidden="true" /></span><span className="min-w-0 flex-1"><span className="block text-sm font-semibold text-[#25372b]">{title}</span><span className="mt-0.5 block text-xs leading-4 text-[#718078]">{detail}</span></span>{alert ? <AlertCircle size={16} className="shrink-0 text-[#b57919]" aria-hidden="true" /> : <Check size={16} className="shrink-0 text-[#378459]" aria-hidden="true" />}</div> }

function Insight({ icon: Icon, number, title, detail }: { icon: typeof Leaf; number: string; title: string; detail: string }) { return <div className="flex min-h-[76px] items-center gap-3 rounded-2xl border border-[#edf3ee] bg-[#f8fbf9] p-3"><span className="text-[11px] font-bold tabular-nums text-[#91aa98]">{number}</span><span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-white text-[#2d7450] shadow-[0_3px_9px_rgba(29,77,46,.04)]"><Icon size={18} aria-hidden="true" /></span><span className="min-w-0"><span className="block text-sm font-semibold text-[#25372b]">{title}</span><span className="mt-0.5 block text-xs leading-4 text-[#718078]">{detail}</span></span></div> }
