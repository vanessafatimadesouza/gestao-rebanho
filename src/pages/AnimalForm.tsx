...
export function AnimalForm() {
  ...
  return <div className="mx-auto max-w-2xl space-y-5">
    <PageHeader backTo={id ? `/animais/${id}` : '/animais'} kicker="Rebanho" title={isEdit ? 'Editar animal' : 'Cadastrar animal'} />
    <form onSubmit={handleSubmit} className="app-surface space-y-6 p-5 sm:p-7">
      ...
      <div className="grid gap-4 sm:grid-cols-2"><div className="sm:col-span-2"><label htmlFor="animal-name" className={labelClass}>Nome do animal *</label><input id="animal-name" type="text" value={name} onChange={e => setName(e.target.value)} required className={inputClass} placeholder="Ex.: Mimosa" autoFocus /></div><div><label htmlFor="animal-sex" className={labelClass}>Sexo *</label><select id="animal-sex" value={sex} onChange={e => setSex(e.target.value as 'M' | 'F')} className={inputClass}><option value="F">Fêmea</option><option value="M">Macho</option></select></div><div><label htmlFor="animal-breed" className={labelClass}>Raça</label><input id="animal-breed" type="text" value={breed} onChange={e => setBreed(e.target.value)} className={inputClass} placeholder="Ex.: Nelore" /></div><div className="sm:col-span-2"><label htmlFor="animal-birth-date" className={labelClass}>Data de nascimento</label><input id="animal-birth-date" type="text" inputMode="numeric" value={birthDate} onChange={e => setBirthDate(formatBirthDate(e.target.value))} className={inputClass} placeholder="dd/mm/aaaa" maxLength={10} /></div></div>

      <div className="rounded-2xl bg-[#f6f9f7] p-4"><div className="mb-4"><p className="text-sm font-bold text-[#24362b]">Filiação</p><p className="text-xs text-[#718078]">Digite um nome ou escolha um animal já cadastrado.</p></div><div className="grid gap-4 sm:grid-cols-2"><div><label htmlFor="animal-mother" className={labelClass}>Mãe</label><input id="animal-mother" list="mother-options" value={motherName} onChange={e => { setMotherName(e.target.value); chooseParent(e.target.value, mothers, setMotherId) }} className={inputClass} placeholder="Digite ou selecione" /><datalist id="mother-options">{mothers.map(mother => <option key={mother.id} value={displayName(mother)} />)}</datalist></div><div><label htmlFor="animal-father" className={labelClass}>Pai</label><input id="animal-father" list="father-options" value={fatherName} onChange={e => { setFatherName(e.target.value); chooseParent(e.target.value, fathers, setFatherId) }} className={inputClass} placeholder="Digite ou selecione" /><datalist id="father-options">{fathers.map(father => <option key={father.id} value={displayName(father)} />)}</datalist></div></div></div>
